...
output "ssm_connect_public" {
  ...
  value       = "aws ssm start-session --target ${module.public_instance.id} --region ${var.region}"
}

output "ssm_connect_private" {
  description = "AWS CLI command to connect to the private instance via SSM"
  value       = "aws ssm start-session --target ${module.private_instance.id} --region ${var.region}"
}
